...
session_start();
 ...
 if(isset($_POST['btn'])){
    ...
    $sql = "INSERT INTO orders (date, sumPrice) VALUES ('$currentDay', $sum)";
    $result = $connect->query($sql); // Use the correct variable name '$connect' instead of '$connnect'
    if($result){
        $order_id=mysqli_insert_id($connect);
        foreach($cart as $key=>$value){
            $sql1="insert into orders_detail(id,product_id,qty,price) values($order_id,$value[id],$value[qty],$value[price])";
